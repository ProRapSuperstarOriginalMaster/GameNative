...
public class SGSRMaterial extends ShaderMaterial {
    ...
    // Fragment shader: SGSR v1 algorithm from Qualcomm's mobile GLSL implementation.
    // BSD-3-Clause: Copyright (c) 2025, Qualcomm Innovation Center, Inc.
    // Note: textureGather component index is hardcoded to 1 (green channel) rather than
    // using the 'mode' variable, because some Adreno GLES drivers require a compile-time
    // constant for the component argument. OperationMode 1 (RGBA) gathers green channel.
    @Override
    protected String getFragmentShader() {
        return "#version 300 es\n" +
               ...
               "\n" +
               "    // Component index is literal 1 (green channel) — required to be a compile-time\n" +
               "    // constant on Adreno GLES drivers. Matches OperationMode 1 (RGBA).\n" +
               ...
               "\n" +
               "        vec4 right   = textureGather(ps0, coord + highp vec2(ViewportInfo[0].x, 0.0), 1);\n" +
               "        vec4 upDown;\n" +
               "        upDown.xy    = textureGather(ps0, coord + highp vec2(0.0, -ViewportInfo[0].y), 1).wz;\n" +
               "        upDown.zw    = textureGather(ps0, coord + highp vec2(0.0,  ViewportInfo[0].y), 1).yx;\n" +
               "\n" +
